...
# 7 executors total: 2 per DN × 4 DNs + 1 on NN
# executor: 5G heap + 1G overhead = 6G per executor (under 13.3GB limit)
SPARK_COMMON = """
        --master yarn \\
        --deploy-mode client \\
        --driver-memory 2G \\
        ...
        --conf spark.sql.shuffle.partitions=140 \\
...
with DAG(
    dag_id="spark_minio_medallion_ML",
    ...
    silver_distributions = BashOperator(
        task_id="silver_distributions",
        bash_command=f"""
        ...
        {S3A_CONF} \
        /home/ubuntu/scripts/silver_distributions.py
        """
    )

    silver_distributions